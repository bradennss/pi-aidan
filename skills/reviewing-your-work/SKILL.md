...
4. Capture the review diff from the repository root in a uniquely named temporary artifact outside the repository. Do not alter the index. Use `git rev-parse --verify HEAD` as the staged-diff base, or the empty tree from `git hash-object -t tree /dev/null` for an unborn branch. Write labeled sections for staged changes from `git diff --cached --no-ext-diff --no-textconv --binary --full-index <base> -- .`, unstaged changes from `git diff --no-ext-diff --no-textconv --binary --full-index -- .`, and untracked files listed by `git ls-files --others --exclude-standard -z`. Append each untracked file with `git diff --no-ext-diff --no-textconv --binary --full-index --no-index -- /dev/null <path>`, accepting exit status 1 as the expected difference result. Build the artifact at a temporary path and move it into place only after every command succeeds. Remove partial files on failure. Record the artifact's absolute path.
