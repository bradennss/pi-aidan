...
3. Decide whether independent review is warranted from the change's size, complexity, blast radius, uncertainty, and risk. Use it by default when a second perspective could catch a material issue, especially for changes to behavior, interfaces, data, security, schemas, dependencies, infrastructure, prompts, or skills. Do not launch independent review for a small, low-risk edit unless the user asks for it. When the user explicitly asks to skip review, treat that request as waiving independent review, honor it, and keep the personal pass and applicable validation. State why independent review was omitted or waived in the final response, report the review as `clean` when the personal pass and validation succeed, and continue with the calling workflow without running steps 4 through 11.
