...
3. Decide whether independent review is required from the change's size, complexity, blast radius, uncertainty, and risk. Require it when a second perspective could catch a material issue, especially for substantive or high-risk changes to behavior, interfaces, data, security, schemas, dependencies, infrastructure, prompts, or skills. This workflow authorizes the independent-review subagent; do not ask the user for separate approval before launching it. Skip independent review for a small, low-risk edit or when the user explicitly waives it. When skipping, keep the personal pass and applicable validation, state why independent review was omitted or waived, report the review as `clean` when those checks succeed, and continue with the calling workflow without running steps 4 through 11.
4. Capture the review diff from the repository root in a uniquely named temporary artifact outside the repository. Do not alter the index. Use `git rev-parse --verify HEAD` as the staged-diff base, or the empty tree from `git hash-object -t tree /dev/null` for an unborn branch. Write labeled sections for staged changes from `git diff --cached --no-ext-diff --no-textconv --binary --full-index <base> -- .`, unstaged changes from `git diff --no-ext-diff --no-textconv --binary --full-index -- .`, and untracked files listed by `git ls-files --others --exclude-standard -z`. Append each untracked file with `git diff --no-ext-diff --no-textconv --binary --full-index --no-index -- /dev/null <path>`, accepting exit status 1 as the expected difference result. Create a temporary directory with `mktemp -d` and keep `XXXXXX` at the end of its template because BSD `mktemp` only replaces a trailing placeholder. Use fixed partial and final filenames inside that directory, then move the partial file to the final artifact only after every diff command succeeds. Remove the temporary directory on failure. Record the artifact's absolute path.
...
9. Stop with `clean` when the reviewer returns no qualifying issues and required validation passes. Remove every temporary diff artifact and its containing temporary directory before the final response.
